...
public abstract class AbstractViolationsBuildAction<T extends AbstractViolationsBuildAction> extends Actionable
    ...
    /**
     * The object to use then the URL is pressed This handles the url's from
     * "${buildpage}/violations/" except for any overridden by do methods in
     * this class (in this instance doGraph).
     *
    ...
    /**
     * The display name for the url to be displayed on the build page right hand
     * side.
     *
    ...
    public T getPreviousResult() {
        ViolationsReport report = ViolationsReport.findViolationsReport(owner);
        return report == null ? null : report.getBuild().getAction((Class<T>) getClass());
    }
