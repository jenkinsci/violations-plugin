...
public class ViolationsMavenReporter extends MavenReporter {
    ...
    /**
     * Called at the end of each maven ?goal?. - so if the maven args are
     * pmd:pmd checkstyle:checkstyle this will get called twice.
     */
    @Override
    ...
    public boolean end(MavenBuild build, Launcher launcher, BuildListener listener) throws InterruptedException,
        ...
        if (workspace == null) {
            ...
            throw new IOException("No workspace for " + build + "; parent workspace: "
                    + (parent != null ? parent.getWorkspace() : "N/A") + "; builtOnStr=" + build.getBuiltOnStr()
                    + "; builtOn=" + build.getBuiltOn());
        }
