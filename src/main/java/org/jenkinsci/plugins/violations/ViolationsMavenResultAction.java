...
public class ViolationsMavenResultAction extends MavenResultAction<ViolationsResult> {
  public ViolationsMavenResultAction(
      final AbstractBuild<?, ?> owner,
      final HealthDescriptor healthDescriptor,
      final String defaultEncoding,
      final ViolationsResult result) {
    super(
        new ViolationsResultAction(owner, healthDescriptor, result), defaultEncoding, "VIOLATIONS");
  }

  @Override
  public MavenAggregatedReport createAggregatedAction(
      final MavenModuleSetBuild build, final Map<MavenModule, List<MavenBuild>> moduleBuilds) {
    return new ViolationsMavenResultAction(
        build,
        getHealthDescriptor(),
        getDefaultEncoding(),
        new ViolationsResult(
            ...
            getDefaultEncoding(),
            new ParserResult(),
            usePreviousBuildAsStable(),
            useOnlyStableBuildsAsReference()));
  }

  @Override
  protected ViolationsResult createResult(
      final ViolationsResult existingResult, final ViolationsResult additionalResult) {
    return new ViolationsReporterResult(
        getOwner(),
        additionalResult.getDefaultEncoding(),
