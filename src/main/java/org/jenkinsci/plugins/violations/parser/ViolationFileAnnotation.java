...
public class ViolationFileAnnotation extends AbstractAnnotation {
 ...
 public ViolationFileAnnotation(final Violation violation, String moduleName) {
  super(toPriority(violation.getSeverity()), violation.getMessage(), violation.getStartLine(), violation.getEndLine(),
    violation.getReporter().name(), violation.getSource().orNull());
