...
public class ReportedFileAsserter {
    ...
    private final ViolationsReportAsserter violationsReportAsserter;
    private final String reportedFile;

    public ReportedFileAsserter(ViolationsReportAsserter violationsReportAsserter, String reportedFile) {
    ...
    private FileCount getOrFail() {
        ImmutableMap<String, FileCount> nameToFileCount = null;
        try {
            Set<FileCount> fileCounts = violationsReportAsserter.getViolationsReport().getModel().getTypeMap()
                    .get(violationsReportAsserter.getTypeDescriptor().getName());
            nameToFileCount = nameToFileCount(fileCounts);
        } catch (Exception e) {
            throw new AssertionError("Unexpected failure when getting fileCounts for \""
                    + violationsReportAsserter.getTypeDescriptor().getName() + "\"", e);
        }
    ...
    public ReportedFileAsserter reportedViolation(int line, String source, String message) {
        for (Violation v : getViolations()) {
            if (v.getLine() == line && v.getSource().equals(source) && v.getMessage().equals(message)) {
                return this;
            }
        }
        fail("Could not find message \"" + message + "\" at " + line + " Found:\n"
    ...
    private Iterable<String> readableStrings(List<Violation> violations) {
        return transform(violations, new Function<Violation, String>() {
            ...
            public String apply(Violation input) {
                return input.getLine() + " Source: " + input.getSource() + " Message: " + input.getMessage();
            }
        });
    }
    ...
    private ImmutableMap<String, FileCount> nameToFileCount(Set<FileCount> fileCounts) {
        checkNotNull(fileCounts, "fileCounts cannot be null!");
