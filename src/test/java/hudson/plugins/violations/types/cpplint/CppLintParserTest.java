...
import static hudson.plugins.violations.types.cpplint.CppLintDescriptor.DESCRIPTOR;
...
public class CppLintParserTest extends HudsonTestCase {
    ...
    public void testParseLineSimple() {
        ...
        Assert.assertEquals("The message is incorrect",
                " Missing space after ,  (3)", violation.getMessage());
        Assert.assertEquals("The violation id is incorrect",
                "whitespace/comma", violation.getViolationId());
        Assert.assertEquals("The line str is incorrect", "64",
                violation.getLineStr());
        Assert.assertEquals("The file name is incorrect",
                "./tests/writestatusmanagertest.cpp", violation.getFileName());
    }
