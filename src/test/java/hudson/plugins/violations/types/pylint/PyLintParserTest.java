...
public class PyLintParserTest extends HudsonTestCase {
    ...
    public void testExtraViolationInfo() {
        ...
        Assert.assertEquals("The message is incorrect",
                "Invalid name \"seasonCount\" (should match [a-z_][a-z0-9_]{2,30}$)", violation.getMessage());
    ...
    public void testThatPylintFileCanBeParsed() throws Exception {
        violationsReport(PYLINT).reportedIn("**/pylint-report.log").perform().assertThat("cpplint.py").wasReported()
                .reportedViolation(710, "W0511", "XXX_HEADER constants defined above.")
                .reportedViolation(1225, "W0511", "TODO(unknown): if delimiter is not None here, we might want to");
    }
}
